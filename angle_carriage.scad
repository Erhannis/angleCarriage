...
difference() {
  ...
  for (i=[0,1]) mirror([i,0,0])
    translate([-B_DIAM/2-ANGLE_THICK/2,0,0]) rotate([0,0,90]) bearingSlot([B_WIDTH,B_DIAM*1.1,B_DIAM*1.1], nub_diam=B_BORE);
  for (j=[0.25,0.75]) translate([WALL_THICK/2+B_WIDTH/2,0,WALL_HEIGHT*j])
  for (i=[0,1]) mirror([0,i,0])
    translate([0,-B_DIAM/2-ANGLE_THICK/2,0]) bearingSlot([B_WIDTH,B_DIAM*1.1,B_DIAM*1.1], nub_diam=B_BORE);
}

!difference() {
  cube([B_WIDTH*3,B_BORE,B_DIAM*3],center=true);
  bearingSlot([B_WIDTH+0.2,B_DIAM*1.1,B_DIAM*1.1], nub_diam=B_BORE, nub_stem=0.1);
}